...
> **Note:** Cache-Control в проекте задаётся через Nitro route rule в `nuxt.config.ts` (`'/api/**': { cache: { maxAge: 60 * 5 } }`). Вызовы `setResponseHeader` в server routes перезаписываются этой route rule. Актуальное значение — `max-age=300` для всех `/api/` эндпоинтов.
...
> **Note:** Фактическое значение Cache-Control для API-эндпоинтов определяется Nitro route rule в `nuxt.config.ts`, а не `setResponseHeader` в server routes. В настоящее время все `/api/` эндпоинты используют `max-age=300` (5 минут).
...
*Последнее обновление: 2026-05-23 — добавлен комментарий о Nitro route rule для Cache-Control (строки 245-247, 714-716)*
